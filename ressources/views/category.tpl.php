    ...
    <div class="container container-articles">
        ...
        <div>
            <?php foreach ($value as $key => $data) { ?>
                <button onclick="window.location.href='?action=blogPostCategory&id=<?= $value[$key]['id'] ?>'"
                        class="btn-primary"><?= $value[$key]['name'] ?></button>
            <?php } ?>
            <button onclick="window.location.href='/'" class="btn-primary">ALL</button>
        </div>
        <?php foreach ($articleByCategory as $key => $value) { ?>
            <div class="card-article">
                <div class="content-card-article" value="<?= $value['id'] ?>"
                     onclick="window.location.href=`/?action=blogPost&id=<?= $value['id'] ?>`">
